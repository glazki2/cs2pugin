...
namespace
{
	...
	void write_test_vpk(const std::filesystem::path& path, const char* extension, const char* directory, const char* name, uint32_t crc,
						uint32_t size)
	{
	...
	void write_extractable_vpk(const std::filesystem::path& path, uint32_t version, uint16_t archive_index, const std::string& preload,
							   const std::string& payload, uint16_t terminator = 0xffffu,
							   uint32_t declared_file_data_size = std::numeric_limits<uint32_t>::max(), const std::string& footer = {})
	{
		...
		const uint32_t file_data_size = declared_file_data_size == std::numeric_limits<uint32_t>::max()
											? (archive_index == 0x7fffu ? static_cast<uint32_t>(payload.size()) : 0u)
											: declared_file_data_size;
		if (version == 1)
		{
	...
	void assert_process_stopped(uint64_t pid)
	{
		...
		if (alive)
		{
			std::fprintf(stderr, "process %llu did not stop\n", static_cast<unsigned long long>(pid));
		}
		assert(!alive);
	}
	...
	void test_vpk(const std::filesystem::path& directory)
	{
		...
		const std::vector<std::filesystem::path> dotted_parent = vpk_path_candidates("vpk:C:\\servers.vpk-backup\\csgo\\pak01.vpk:maps\\de_test.vpk");
	...
	void test_subprocess(const std::filesystem::path& directory, const std::filesystem::path& executable)
	{
		...
		std::string error;
		assert(run_process(executable, {"--process-probe", "space [test]"}, std::chrono::seconds(5), nullptr, false, posix_process_group::isolated,
						   result, error));
		...
		assert(run_process(executable, {"--process-flood"}, std::chrono::seconds(5), nullptr, false, posix_process_group::isolated, result, error));
		...
		assert(run_process(executable, {"--process-touch", unicode_output}, std::chrono::seconds(5), nullptr, false, posix_process_group::isolated,
						   result, error));
		assert(result.exit_code == 0 && read_text_file(unicode_output) == "touched");
#endif

		assert(run_process(executable, {"--process-sleep"}, std::chrono::milliseconds(50), nullptr, false, posix_process_group::isolated, result,
						   error));
		...
		assert(run_process(executable, {"--process-sleep"}, std::chrono::seconds(5), &cancel, false, posix_process_group::isolated, result, error));
		canceller.join();
		assert(result.cancelled);
		assert(result.output_tail.find("sleep probe") != std::string::npos);

		const std::filesystem::path nested_pid = directory / "nested-timeout.pid";
		assert(run_process(executable, {"--process-nested", nested_pid}, std::chrono::seconds(1), nullptr, false, posix_process_group::isolated,
						   result, error));
		assert(result.timed_out);
		...
		assert(run_process(executable, {"--process-nested", cancelled_pid}, std::chrono::seconds(5), &cancel, false, posix_process_group::isolated,
						   result, error));
	...
	void write_test_glb(const std::filesystem::path& path, const std::string& surface_property, bool include_surface_property = true)
	{
		...
		std::string json =
			R"({"asset":{"version":"2.0"},"buffers":[{"byteLength":42}],"bufferViews":[{"buffer":0,"byteOffset":0,"byteLength":36},{"buffer":0,"byteOffset":36,"byteLength":6}],"accessors":[{"bufferView":0,"componentType":5126,"count":3,"type":"VEC3","max":[1,1,0],"min":[0,0,0]},{"bufferView":1,"componentType":5123,"count":3,"type":"SCALAR"}],"meshes":[{"primitives":[{"attributes":{"POSITION":0},"indices":1}]}],"nodes":[{"mesh":0,"extras":{)"
			+ surface_json + R"("InteractAs":[]}}],"scenes":[{"nodes":[0]}],"scene":0})";
	...
	void test_bvh(const std::filesystem::path& directory)
	{
		...
		for (int x = -4; x <= 4; ++x)
		{
			for (int y = -4; y <= 4; ++y)
			{
				triangles.push_back({{static_cast<float>(x), static_cast<float>(y), 0},
									 {static_cast<float>(x + 1), static_cast<float>(y), 0},
									 {static_cast<float>(x), static_cast<float>(y + 1), 0}});
				triangles.push_back({{static_cast<float>(x + 1), static_cast<float>(y + 1), 0},
									 {static_cast<float>(x), static_cast<float>(y + 1), 0},
									 {static_cast<float>(x + 1), static_cast<float>(y), 0}});
			}
		}
		...
		for (bvh8_node& node : invalid_tree.nodes)
		{
			for (uint32_t& child : node.child)
			{
				...
				if (shared_packet == k_invalid_ref)
				{
					shared_packet = child;
				}
				else
		...
		assert(error == "BVH8 contains unreachable nodes or packets");
		invalid_tree = loaded;
		invalid_tree.header.max_depth =
			invalid_tree.header.max_depth < k_max_tree_depth ? invalid_tree.header.max_depth + 1u : invalid_tree.header.max_depth - 1u;
		assert(!validate_bvh8(invalid_tree, error));
		...
		for (int i = 0; i < 5000; ++i)
		{
			...
			{
				expected = expected || scalar_hit(value, origin, target);
			}
