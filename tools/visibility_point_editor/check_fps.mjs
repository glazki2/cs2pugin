...
for (const [source, pattern, name] of [
	...
	[nativeSampling, /k_horizontal_bounds_padding\s*=\s*32\.0f/, "AABB side padding"],
...
assert.equal(wallResult.stage, "capsule-blocked");
...
assert.equal(smokeResult.stage, "capsule-blocked");
...
assert.equal(capsulePriorityResult.stage, "capsule-visible",
	"the clear result must report the capsule silhouette as the deciding stage");
...
assert.equal(muzzleFallback.stage, "muzzle-visible",
	"a clear muzzle fallback must be reported as the deciding stage");
...
aabbFallbackMap.segment_blocked = (_start, end) =>
	({blocked: Math.abs(end.x - 52) > 0.001, packet: 0});
...
assert.equal(aabbFallback.stage, "aabb-visible",
	"a clear AABB fallback must be reported as the deciding stage");
...
assert.deepEqual(Array.from(standingAabb.slice(0, 3)), [-38, -28, 30],
	"runtime AABB must use 32-unit side padding");
...
assert.ok(Math.abs(alignedPoint.x - 152) < 0.001 && Math.abs(alignedPoint.y - 2) < 0.001,
