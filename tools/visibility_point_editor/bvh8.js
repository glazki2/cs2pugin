...
export function shoulder_offset(pingMs, tuning = {}, movementIntent = true)
{
	...
	const configuredBase = Math.min(256, Math.max(0, Number.isFinite(requestedBase) ? requestedBase : 64));
	const base = movementIntent ? configuredBase : configuredBase * 0.5;
	const scalePerMs = Math.min(4, Math.max(0, Number.isFinite(requestedScale) ? requestedScale : 0.64));
	const maximum = Number.isFinite(requestedMaximum) ? requestedMaximum : 0;
	const stepped = Math.floor(Math.max(0, pingMs) / 25) * 25;
	const wanted = base + stepped * scalePerMs;
	return maximum > 0 ? Math.min(Math.max(base, maximum), wanted) : wanted;
}
...
export function runtime_origins(map, viewer, traversal = null)
{
	...
	const leftButton = Boolean(viewer.buttons?.a);
	const rightButton = Boolean(viewer.buttons?.d);
	const leftOffset = shoulder_offset(viewer.pingMs, viewer.tuning,
		leftButton);
	const rightOffset = shoulder_offset(viewer.pingMs, viewer.tuning,
		rightButton);
	const eye = viewer.eye || add(viewer.origin, {x: 0, y: 0, z: Number(viewer.eyeHeight) || 64});
	const origins = [];
	add_unique(origins, eye);
	add_unique(origins, safe_origin(map, eye, subtract(eye, scale(right, leftOffset)), traversal));
	add_unique(origins, safe_origin(map, eye, add(eye, scale(right, rightOffset)), traversal));
	...
	if (forwardInput !== 0)
	{
		...
		const movementOffset = shoulder_offset(viewer.pingMs, viewer.tuning, true);
		direction = scale(direction, movementOffset / length);
