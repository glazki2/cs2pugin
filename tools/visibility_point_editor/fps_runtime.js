...
export function target_aabb(bot)
{
	...
	return new Float32Array([
		bot.origin.x - 48, bot.origin.y - 48, bot.origin.z,
		bot.origin.x + 48, bot.origin.y - 48, bot.origin.z,
		bot.origin.x - 48, bot.origin.y + 48, bot.origin.z,
		bot.origin.x + 48, bot.origin.y + 48, bot.origin.z,
		bot.origin.x - 48, bot.origin.y - 48, bot.origin.z + height + 8,
		bot.origin.x + 48, bot.origin.y - 48, bot.origin.z + height + 8,
		bot.origin.x - 48, bot.origin.y + 48, bot.origin.z + height + 8,
		bot.origin.x + 48, bot.origin.y + 48, bot.origin.z + height + 8
	]);
...
export function trace_capsule_target(map, viewer, targetSet, options = {})
{
	...
	if (options.held)
	{
		return {origins: originValues, rays: new Float32Array(), blocked: new Uint8Array(), clearCount: 0,
			...
			smokeBlocked: false, stage: "held", stageDetail: "reveal hold reused",
			cache: {packets, occluders}, ...stats, traversal: null};
	}
	let rawVisible = !valid;
	let indeterminate = !valid;
	let wallBlocked = false;
	let smokeBlocked = false;
	let stage = valid ? "capsule" : "fail-open";
	let stageDetail = valid ? "checking 19 capsule volumes" : "invalid capsule capture";
	for (let originIndex = 0; valid && !rawVisible && originIndex < origins.length; ++originIndex)
	{
		...
		if (query.result !== "blocked")
		{
			...
			stage = query.stage || (indeterminate ? "fail-open" : "capsule-visible");
			stageDetail = indeterminate ? "runtime fail-open" : "capsule volume visible";
			break;
		}
		wallBlocked ||= query.reason === "wall";
		smokeBlocked ||= query.reason === "smoke";
		for (let fallbackIndex = 0; fallbackIndex < fallbacks.length; ++fallbackIndex)
		{
			const point = fallbacks[fallbackIndex];
			...
			{
				rawVisible = true;
				stage = fallbackIndex < 8 ? "aabb-visible" : "muzzle-visible";
				stageDetail = fallbackIndex < 8
					? `AABB corner ${fallbackIndex + 1}/8 clear`
					: "muzzle ray clear";
				break;
			}
		}
	...
	return {
		...
		wallBlocked,
		smokeBlocked,
		stage: rawVisible ? stage : (wallBlocked || smokeBlocked ? "hidden" : "fail-open"),
		stageDetail: rawVisible ? stage : (wallBlocked || smokeBlocked ? "all runtime probes blocked" : "runtime fail-open"),
