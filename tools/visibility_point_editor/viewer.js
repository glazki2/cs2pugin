...
const k_shoulder_offset = 64;
const k_vertical_origin_offset = 16;
const k_horizontal_bounds_padding = 32;
...
let decision_stage = "no-map";
let decision_detail = "No BVH8 loaded; no runtime wall decision is running.";
let studio_ground;
let studio_grid;
...
function decision_short(stage)
{
	return stage === "capsule-visible" ? "Capsule" : stage === "aabb-visible" ? "AABB" :
		stage === "muzzle-visible" ? "Muzzle" : stage === "capsule-blocked" ? "Capsule blocked" :
		stage === "hidden" ? "Hidden" : stage === "held" ? "Held" : stage === "fail-open" ? "Fail-open" :
		stage === "waiting" ? "Waiting" : stage === "capsule" ? "Capsules" : "No BVH8";
}

function decision_message(stage, detail = "")
{
	const message = stage === "capsule" ? "Checking the 19 animated capsule volumes (no center-point rays)." :
		stage === "capsule-visible" ? "Visible from the capsule silhouette; no fallback ray was needed." :
		stage === "capsule-blocked" ? "Capsule silhouette was blocked; AABB and muzzle fallbacks may have run." :
		stage === "aabb-visible" ? "Visible from a clear padded AABB corner fallback." :
		stage === "muzzle-visible" ? "Visible from the clear muzzle fallback ray." :
		stage === "hidden" ? "Capsules and every fallback ray were blocked." :
		stage === "held" ? "Previous reveal is being held; LOS work is skipped." :
		stage === "fail-open" ? "Runtime would fail open because capture or the time budget was incomplete." :
		stage === "waiting" ? "Place both players to run the runtime decision." :
		"No BVH8 loaded; capsule geometry is previewed but visibility is not decided.";
	const usefulDetail = detail && ![
		"checking 19 capsule volumes", "capsule volume visible", "runtime fail-open",
		"muzzle ray clear", "all runtime probes blocked", "reveal hold reused", "invalid capsule capture"
	].includes(detail);
	return usefulDetail ? `${message} ${detail}` : message;
}
...
function draw_runtime_rays()
{
	if (map_metadata)
	{
		if (!map_simulation_ready())
		{
			...
			blocked_ray_count = 0;
			wall_visible = null;
			decision_stage = "waiting";
			decision_detail = decision_message("waiting");
			update_status();
			return;
		}
		request_map_trace();
	...
	ray_lines = null;
	ray_count = 0;
	decision_stage = "no-map";
	decision_detail = decision_message("no-map");
	// Without a BVH8 map there is no wall query. Drawing fallback lines here
	// would imply a runtime decision that never happened.
	for (const origin of stationary_viewer_origins()) origin_group.add(make_marker(origin, k_origin_color, k_aabb_dot_radius));
}
...
function draw_map_trace(origins, rays, blocked, visible, clearCount, stage = "hidden", detail = "")
{
	...
	decision_detail = decision_message(stage, detail);
...
function remember_trace_result(rays, visible, clearCount, stage = "hidden", detail = "")
{
	ray_count = rays.length / 6;
	clear_ray_count = clearCount;
	blocked_ray_count = ray_count - clearCount;
...
function update_status()
{
	...
	const wallLabel = wall_visible === null ? (map_metadata ? "Place players" : "No map")
		: decision_stage === "fail-open" ? "Fail-open" : decision_stage === "held" ? "Held"
			: (wall_visible ? "Visible" : "Hidden");
	$("status-wall-result").textContent = wallLabel;
	$("status-wall-result").style.color = wall_visible === null ? ""
		: decision_stage === "fail-open" ? "#b7791f" : (wall_visible ? "#13784a" : "#b90f20");
	$("status-decision").textContent = decision_short(decision_stage);
	$("decision-detail").textContent = decision_detail || decision_message(decision_stage);
...
function unload_map(message = "Map unloaded. Static Studio geometry restored.")
{
	leave_play_mode();
	++map_load_id;
		...
		else if (message.type === "traced" && message.id === trace_id)
		{
			...
			if (map_simulation_ready() && message.results?.length)
			{
				...
				draw_map_trace(primary.origins, primary.rays, primary.blocked, primary.visible, primary.clearCount,
					primary.stage, primary.stageDetail);
...
function handle_play_state(state)
{
	...
	if (drawDebug)
	{
		...
		draw_map_trace(state.visibility.origins, state.visibility.rays, state.visibility.blocked,
			state.visibility.visible, state.visibility.clearCount, state.visibility.stage, state.visibility.stageDetail);
	...
	else
	{
		remember_trace_result(state.visibility.rays, state.visibility.visible, state.visibility.clearCount,
			state.visibility.stage, state.visibility.stageDetail);
	}
	...
	$("play-blocker").textContent = visibilities.some((visibility) => visibility.stage === "fail-open") ? "fail-open"
		: visibilities.every((visibility) => visibility.visible) ? "clear"
	...
	for (const visibility of visibilities)
		stages.set(decision_short(visibility.stage), (stages.get(decision_short(visibility.stage)) || 0) + 1);
	$("play-decision").textContent = [...stages.entries()].map(([stage, count]) => `${stage} ${count}`).join(" · ") || "waiting";
	update_play_visibility();
}
...
function update_range_labels()
{
	...
	$("viewer-ping-value").textContent = `${Math.round(read_number("viewer-ping"))} ms / ${format_number(shoulder_offset(read_number("viewer-ping"), runtime_tuning(), false))} idle units`;
}
...
function run_self_checks()
{
	...
	expect(generated_aabb_points()[0].x === -48 && generated_aabb_points()[7].z === 80, "runtime AABB padding");
	...
	expect(shoulder_offset(0, {}, false) === 32 && shoulder_offset(200, {}, false) === 160
		&& shoulder_offset(300, {}, false) === 224 && shoulder_offset(0, {}, true) === 64
		&& shoulder_offset(200, {}, true) === 192 && shoulder_offset(300, {}, true) === 256,
		"runtime ping shoulder range");
