...
export function capsule_visible_from_origin(map, origin, capsuleValues, options = {})
{
	...
	if (!capsules || !finite_vec(origin)) return {result: "indeterminate", stage: "fail-open", stats, rays, blocked};
	...
	for (const capsule of capsules)
	{
		if (point_segment_distance_sq(origin, capsule.start, capsule.end) <= capsule.radius ** 2)
			return {result: "visible", stage: "capsule-visible", stats, rays, blocked};
	...
	if (!view) return {result: "indeterminate", stage: "fail-open", stats, rays, blocked};
	...
	if (projections.some((projection) => !projection)) return {result: "indeterminate", stage: "fail-open", stats, rays, blocked};
	...
	if (!mapResult.complete)
		return {result: "indeterminate", stage: "fail-open", stats, rays, blocked};
	if (mapResult.occluded)
		return {result: "blocked", stage: "capsule-blocked", reason: "wall", stats, rays, blocked, occluderCache: mapResult.cache};
	...
	if (!smokeActive)
	{
		for (let index = 0; index < capsules.length; ++index)
		{
			...
			if (result === "visible")
				return {result: "visible", stage: "capsule-visible", stats, rays, blocked, occluderCache: mapResult.cache};
			if (result !== "occluded")
				return {result: "indeterminate", stage: "fail-open", stats, rays, blocked, occluderCache: mapResult.cache};
		}
		return {result: "blocked", stage: "capsule-blocked", reason: "wall", stats, rays, blocked, occluderCache: mapResult.cache};
	}
	...
	if (geometryVisibleSample)
		return {result: "blocked", stage: "capsule-blocked", reason: "smoke", stats, rays, blocked, occluderCache: mapResult.cache};
	for (let index = 0; index < capsules.length; ++index)
	{
		if (now_ms() >= deadline)
			return {result: "indeterminate", stage: "fail-open", stats, rays, blocked, occluderCache: mapResult.cache};
