...
namespace cs2fow
{
	...
	void visibility_worker::run_helper(uint32_t worker_index)
	{
		...
		for (;;)
		{
			...
			{
				std::unique_lock lock(mutex_);
				condition_.wait(lock, [&] { return stopping_.load() || (active_job_ != nullptr && job_generation_ != observed_generation); });
				if (stopping_.load())
				{
					return;
				}
	...
	void visibility_worker::process(job& current, uint32_t worker_index)
	{
		...
		job_totals totals;
		const smoke_snapshot* active_smokes = current.snapshot.smoke_enabled && current.snapshot.smoke_available && current.snapshot.smokes != nullptr
													  && !current.snapshot.smokes->volumes.empty()
												  ? current.snapshot.smokes.get()
												  : nullptr;
		for (;;)
		{
			if (stopping_.load())
			...
			for (uint32_t target_ticket = 0; target_ticket < k_max_players; ++target_ticket)
			{
				...
				if (!visibility_pair_enabled(recipient, target, from, to, current.snapshot.filter_teammates))
				{
					continue;
				}
				...
				for (uint32_t origin_index = 0; blocked && origin_index < ray_origins.count; ++origin_index)
				{
					...
					totals.max_rebuilt_proof_leaves = std::max(totals.max_rebuilt_proof_leaves, query_stats.max_rebuilt_proof_leaves);
	...
	void visibility_worker::publish(job& current)
	{
		...
		for (uint32_t index = 0; index < thread_count_; ++index)
		{
			...
			result.max_rebuilt_proof_leaves = std::max(result.max_rebuilt_proof_leaves, totals.max_rebuilt_proof_leaves);
