...
namespace cs2fow
{
	...
	void automatic_baker::run(bake_request request)
	{
		...
		if (request.map != header.map_name || header.flags != request.source.flags || header.source_crc32 != request.source.metadata.crc32
			|| header.source_size != request.source.metadata.size)
		{
			completion.data = {};
