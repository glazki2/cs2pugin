...
namespace cs2fow
{
	...
	{
		return left.has_controller != right.has_controller || left.hltv != right.hltv || left.pawn_entity != right.pawn_entity
			   || left.team != right.team || left.alive != right.alive || left.spawning != right.spawning || left.death_flags != right.death_flags
			   || left.has_death_info != right.has_death_info || left.death_time != right.death_time || left.death_info_time != right.death_info_time;
	}
	...
	inline void update_lifecycle_guard(lifecycle_guard& guard, const lifecycle_key& key, bool stable, std::chrono::steady_clock::time_point now,
	...
	inline bool update_pair_guard(pair_guard& guard, const lifecycle_key& recipient_key, bool recipient_stable, const lifecycle_key& target_key,
								  bool target_stable)
	{
	...
	template<typename handle_type, size_t max_count>
	inline void hidden_group_store(hidden_entity_group<handle_type, max_count>& group, const hidden_entity_group<handle_type, max_count>& source,
