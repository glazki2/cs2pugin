...
namespace cs2fow
{
	...
	bool run_process(const std::filesystem::path& executable, const std::vector<std::filesystem::path>& arguments, std::chrono::milliseconds timeout,
					 const std::atomic_bool* cancel, bool low_priority, posix_process_group process_group, process_result& result, std::string& error)
	{
		result = {};
		(void)process_group;
		...
		if (!CreatePipe(&output_read, &output_write, &pipe_security, 0) || !SetHandleInformation(output_read, HANDLE_FLAG_INHERIT, 0))
		{
		...
		for (const std::filesystem::path& argument : arguments)
		{
			values.push_back(argument.native());
		}
		...
		if (setup_error == 0)
		{
			setup_error = posix_spawn_file_actions_adddup2(&file_actions, output_pipe[1], STDOUT_FILENO);
		}
		if (setup_error == 0)
		{
			setup_error = posix_spawn_file_actions_adddup2(&file_actions, output_pipe[1], STDERR_FILENO);
		}
		if (setup_error == 0)
		{
			setup_error = posix_spawn_file_actions_addclose(&file_actions, output_pipe[0]);
		}
		if (setup_error == 0)
		{
			setup_error = posix_spawn_file_actions_addclose(&file_actions, output_pipe[1]);
		}
		if (setup_error != 0)
		{
		...
		int status = 0;
		bool output_ok = true;
		const auto terminate = [&] { kill(process_group == posix_process_group::isolated ? -pid : pid, SIGKILL); };
		for (;;)
		{
			output_ok = drain_output() && output_ok;
