...
namespace cs2fow
{
	namespace
	{
		...
		bool validate_header(const bvh8_header& header, uint64_t actual_size, std::string& error)
		{
			...
			if (header.node_count == 0 || header.packet_count == 0 || header.triangle_count == 0 || header.max_depth == 0
				|| header.max_depth > k_max_tree_depth || header.triangle_count > static_cast<uint64_t>(header.packet_count) * 8u)
			{
			...
			if (!checked_add(expected_packets, packet_bytes, expected_size) || header.nodes_offset != expected_nodes
				|| header.packets_offset != expected_packets || header.file_size != expected_size || actual_size != expected_size
		...
		bool replace_file(const std::filesystem::path& temporary, const std::filesystem::path& destination, std::string& error)
		{
			...
			{
				error =
					"could not atomically install BVH8 file: " + std::error_code(static_cast<int>(GetLastError()), std::system_category()).message();
				return false;
			}
#else
	...
	bool validate_bvh8(const bvh8_data& data, std::string& error)
	{
		...
		if (!validate_header(header, header.file_size, error) || header.node_count != data.nodes.size() || header.packet_count != data.packets.size())
		{
			if (error.empty())
			{
	...
	bool load_bvh8(const std::filesystem::path& path, bvh8_data& data, std::string& error, const std::atomic_bool* cancel)
	{
		error.clear();
		data = {};
		...
		if (!validate_bvh8(data, error) || cancelled() || !payload_crc(data, checksum, cancel) || checksum != header.payload_crc32)
		{
			if (error.empty())
			{
				error = cancel != nullptr && cancel->load() ? "BVH8 load cancelled" : "BVH8 payload CRC mismatch";
			}
			data = {};
			return false;
		}
	...
	bool write_bvh8(const std::filesystem::path& path, bvh8_data& data, std::string& error)
	{
		...
		stream.write(reinterpret_cast<const char*>(data.packets.data()),
					 static_cast<std::streamsize>(data.packets.size() * sizeof(triangle_packet8)));
		stream.flush();
		...
		if (!load_bvh8(temporary, verified, error) || verified.header.payload_crc32 != data.header.payload_crc32)
		{
			if (error.empty())
			{
				error = "temporary BVH8 validation failed";
			}
			std::filesystem::remove(temporary, filesystem_error);
			return false;
		}
