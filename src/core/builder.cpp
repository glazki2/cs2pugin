...
namespace cs2fow
{
	namespace
	{
				...
				: output_(output), packet_triangle_sources_(packet_triangle_sources)
			{
				...
				for (const triangle& item : input)
				{
					...
					triangles_.push_back({item,
										  box,
										  {(item.v0.x + item.v1.x + item.v2.x) / 3.0f, (item.v0.y + item.v1.y + item.v2.y) / 3.0f,
										   (item.v0.z + item.v1.z + item.v2.z) / 3.0f}});
				}
		...
		private:
			...
			split best_split(range item) const
			{
				...
				for (uint32_t axis = 0; axis < 3; ++axis)
				{
					...
					for (uint32_t i = item.begin; i < item.end; ++i)
					{
						...
						const uint32_t bin =
							std::min(k_bin_count - 1u, static_cast<uint32_t>((component(current.centroid, axis) - centroid_min) * scale));
						++counts[bin];
					...
					for (uint32_t i = 0; i + 1u < k_bin_count; ++i)
					{
						...
						const float cost =
							area(left_box[i]) * static_cast<float>(left_count[i]) + area(right_box[i + 1u]) * static_cast<float>(right_count[i + 1u]);
						const float gain = parent_cost - cost;
						if (!best.valid || gain > best.gain)
						{
							best = {true, axis, i,
									centroid_min + (centroid_max - centroid_min) * static_cast<float>(i + 1u) / static_cast<float>(k_bin_count),
									gain};
						}
					}
				}
			...
			std::pair<range, range> apply_split(range item, split choice)
			{
				...
				auto middle =
					std::partition(first, last, [&](uint32_t index) { return component(triangles_[index].centroid, choice.axis) < choice.position; });
	...
	bool build_bvh8(std::span<const triangle> triangles, bvh8_data& result, std::string& error, std::vector<uint32_t>* packet_triangle_sources)
	{
		result = {};
