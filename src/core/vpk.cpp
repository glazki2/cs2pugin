...
namespace cs2fow
{
	namespace
	{
		...
		bool read_header(std::ifstream& stream, vpk_header_info& info, std::string& error)
		{
			...
			if (!read_value(stream, signature) || !read_value(stream, version) || !read_value(stream, tree_size) || signature != k_vpk_signature
				|| (version != 1 && version != 2))
			{
		...
		bool read_string(std::ifstream& stream, uint64_t tree_end, std::string& value)
		{
			...
			for (;;)
			{
				...
				if (position < 0 || static_cast<uint64_t>(position) >= tree_end)
				{
					return false;
				}
				char character = '\0';
		...
		bool copy_range(std::ifstream& source, uint64_t offset, uint64_t length, std::ofstream& destination, uint32_t& crc, std::string& error)
		{
		...
		for (;;)
		{
			...
			for (;;)
			{
				...
				for (;;)
				{
					...
					if (!read_value(stream, entry.crc32) || !read_value(stream, entry.preload_size) || !read_value(stream, entry.archive_index)
						|| !read_value(stream, entry.archive_offset) || !read_value(stream, archive_length) || !read_value(stream, terminator)
					...
					entry.path = (directory.empty() ? name : directory + "/" + name) + (extension.empty() ? "" : "." + extension);
					...
					if (entry.archive_index == k_embedded_archive
						&& (!checked_add(entry.archive_offset, entry.archive_size, embedded_end) || embedded_end > header.embedded_data_size))
					{
	...
	bool find_vpk_entry(std::span<const vpk_entry> entries, std::string_view entry_path, vpk_entry& entry, std::string& error)
	{
		const std::string wanted = normalized(std::string(entry_path));
		const auto found = std::find_if(entries.begin(), entries.end(), [&](const vpk_entry& candidate) { return candidate.path == wanted; });
	...
	bool find_vpk_entry(const std::filesystem::path& vpk_path, const std::string& entry_path, vpk_entry& entry, std::string& error)
	{
		...
		return list_vpk_entries(vpk_path, entries, error) && find_vpk_entry(std::span<const vpk_entry>(entries), entry_path, entry, error);
	}
	...
	bool extract_vpk_entry(const std::filesystem::path& vpk_path, const vpk_entry& entry, const std::filesystem::path& output, std::string& error)
	{
			...
			|| !range_fits(vpk_path, entry.preload_offset, entry.preload_size, error))
		{
			if (error.empty())
			{
				error = "invalid VPK entry metadata";
			}
			return false;
		}
		...
		if (entry.archive_size != 0)
		{
			...
			uint64_t embedded_end = 0;
			if (entry.archive_index == k_embedded_archive
				&& (!checked_add(entry.archive_offset, entry.archive_size, embedded_end) || embedded_end > entry.embedded_data_size))
			{
				error = "VPK embedded entry exceeds the file-data section";
				return false;
			}
			if (!checked_add(archive_base, entry.archive_offset, archive_offset) || !range_fits(archive, archive_offset, entry.archive_size, error))
			{
				return false;
			}
		}
		...
		if (entry.archive_size != 0)
		{
			archive_stream.open(archive, std::ios::binary);
		}
			...
			|| (entry.archive_size != 0 && !copy_range(archive_stream, archive_offset, entry.archive_size, destination, crc, error)))
		{
			if (error.empty())
			{
				error = "could not open VPK extraction files";
			}
			destination.close();
