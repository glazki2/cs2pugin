...
namespace cs2fow
{
	namespace
	{
		...
		{
			const vec3 gltf {matrix[0] * source[0] + matrix[4] * source[1] + matrix[8] * source[2] + matrix[12],
			...
			return {gltf.z * k_source_units_per_meter, gltf.x * k_source_units_per_meter, gltf.y * k_source_units_per_meter};
		}
							...
							std::vector<std::string>* triangle_surfaces)
	{
		...
		for (cgltf_size node_index = 0; node_index < data->nodes_count; ++node_index)
		{
			...
			for (cgltf_size primitive_index = 0; primitive_index < node.mesh->primitives_count; ++primitive_index)
			{
				...
				if (primitive.type != cgltf_primitive_type_triangles || positions == nullptr || primitive.indices == nullptr
