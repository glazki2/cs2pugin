...
namespace cs2fow
{
	namespace
	{
		...
		bool parse_arguments(std::span<const std::filesystem::path> argv, arguments& result)
		{
			for (size_t i = 1; i < argv.size(); ++i)
			{
						 ...
						 && i + 1 < argv.size())
				{
					...
					else if (option == "--debug-obj")
					{
						result.debug_obj = value;
					}
					else
					{
						result.studio_surfaces = value;
					}
				}
				else
				{
		...
		bool extract_nested_map(const map_source& source, const std::filesystem::path& temporary, std::filesystem::path& map_vpk, std::string& error)
		{
						  ...
						  const import_report& report, const bvh8_data& data, std::string& error)
		{
			...
			for (size_t i = 0; i < report.groups.size(); ++i)
			{
				...
				stream << "    {\"name\": \"" << json_escape(group.name) << "\", \"accepted\": " << (group.accepted ? "true" : "false")
					   << ", \"triangles\": " << group.triangles << ", \"surface_property\": \"" << json_escape(group.surface_property)
					   << "\", \"tags\": [";
		...
		int run(std::span<const std::filesystem::path> argv)
		{
			...
			if (!args.inspect_bvh8.empty())
			{
				...
				std::cout << "{\"map\":\"" << json_escape(header.map_name) << "\",\"source_kind\":\""
						  << (header.flags == 0 ? "world_physics" : "nested_map_vpk") << "\",\"source_crc32\":\"0x" << std::hex << std::setw(8)
						  << std::setfill('0') << std::nouppercase << header.source_crc32 << std::dec << "\",\"source_size\":" << header.source_size
						  << ",\"triangles\":" << header.triangle_count << ",\"nodes\":" << header.node_count
						  << ",\"packets\":" << header.packet_count << ",\"max_depth\":" << header.max_depth << "}\n";
				return 0;
			}
			if (args.list_maps)
			{
